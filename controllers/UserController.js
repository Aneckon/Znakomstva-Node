...
class UserController {
    ...
    async addInfo(req, res) {
        const {id, gender, height, weight, hairColor, eyeColor, datePurpose, aboutMyself} = req.body
        const {mainProfilePicture} = req.files
        let filename = uuid.v4() + '.jpg'
        mainProfilePicture.mv(path.resolve(__dirname, '..', 'static', filename))
        const user = await User.update({gender, height, weight, hairColor, eyeColor, datePurpose, aboutMyself, mainProfilePicture: filename}, {where: {id}})
        return res.json(user)
    }
